...
function initializeMusic() {
    ...
    if (heartButton) {
        heartButton.addEventListener('click', function(e) {
            // Cria uma chuva intensa de corações
            createIntenseHeartRain();
            
            // Efeito de escala no coração principal
            ...
            music.play().catch(error => {
                console.log('Reprodução bloqueada. Tente interagir com a página primeiro.');
            });
            
            // Inicia a sequência principal após a chuva de corações
            setTimeout(() => {
                startMainSequence();
            }, 2000);
